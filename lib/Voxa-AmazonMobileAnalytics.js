...
function register(skill, config) {
  ...
  skill.onSessionEnded((voxaEvent) => {
    console.log('voxaEvent', voxaEvent);
    if (voxaEvent.request.type === 'SessionEndedRequest') {
      if (voxaEvent.request.reason === 'ERROR') {
        voxaEvent.ama.logEvents('Session ended with error', { Error: voxaEvent.request.error });
      } else {
        voxaEvent.ama.logEvents('Session ended');
      }
